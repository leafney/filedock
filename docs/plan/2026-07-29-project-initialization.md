...
26. 作为项目负责人，我希望每个阶段都有独立提交，以便查看阶段边界和回滚点。
...
- Vite 默认端口使用 `5173`。
- `/version` 代理到 `http://127.0.0.1:8195`。
...
低智能编码模型必须严格按以下顺序执行。每个任务完成后先验证，再创建独立 Git 提交，然后自动进入下一任务，不等待额外确认。
...
- 不复制通用包，不写服务器。
- 完成配置测试并执行对应测试。
- 阶段提交：`chore(architecture): 初始化根工程与配置骨架`。
...
- 阶段提交：`feat(storage): 接入 SQLite 基础连接`。
...
- 阶段提交：`docs(project): 完成架构初始化文档与验收`。
...
- 本 PRD 已获用户批准，按阶段顺序实施并在每阶段完成后提交。
