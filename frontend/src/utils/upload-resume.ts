...
export function findUploadResume(roomCode: string, file: Pick<File, "name" | "size" | "lastModified">, now = Date.now(), storage = browserStorage()): UploadResumeRecord | undefined {
  ...
  return records.filter((record) => record.fileName === file.name && record.fileSize === file.size && record.lastModified === file.lastModified).sort((left, right) => right.createdAt - left.createdAt)[0];
}
...
function parseRecord(raw: string | null): UploadResumeRecord | undefined {
  ...
  try {
    ...
    return value as UploadResumeRecord;
  } catch {
    return undefined;
  }
}
